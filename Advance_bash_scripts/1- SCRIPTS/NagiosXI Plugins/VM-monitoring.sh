#!/bin/bash
# Auther: john 
# Created date: 10 April 2014 - 05 May 2014.
...
if [ `$VMRUN list | grep -w $VM_0 |wc -l` -eq "1" ]; then 
	VM_0_STATE="ON"
	VM_0_PERF="1" 
	...
	VM_0_DEC_CPU="$(grep 'Cpu:' "$VM_0_OUT" | awk '{print $2}')"
...
if [ `$VMRUN list | grep -w $VM_1 |wc -l` -eq "1" ]; then 
	VM_1_STATE="ON"
	VM_1_PERF="1"
	...
	VM_1_DEC_CPU="$(grep 'Cpu:' "$VM_1_OUT" | awk '{print $2}')"
...
if [ `$VMRUN list | grep -w $VM_2 |wc -l` -eq "1" ]; then 
	VM_2_STATE="ON"
	VM_2_PERF="1"
	...
	VM_2_DEC_CPU="$(grep 'Cpu:' "$VM_2_OUT" | awk '{print $2}')"
...
if [ `$VMRUN list | grep -w $VM_3 |wc -l` -eq "1" ]; then 
	VM_3_STATE="ON"
	VM_3_PERF="1"
	...
	VM_3_DEC_CPU="$(grep 'Cpu:' "$VM_3_OUT" | awk '{print $2}')"
...
if [ `$VMRUN list | grep -w $VM_4 |wc -l` -eq "1" ]; then 
	VM_4_STATE="ON"
	VM_4_PERF="1"
	...
	VM_4_DEC_CPU="$(grep 'Cpu:' "$VM_4_OUT" | awk '{print $2}')"
...
if [ `$VMRUN list | grep -w $VM_5 |wc -l` -eq "1" ]; then 
	VM_5_STATE="ON"
	VM_5_PERF="1"
	...
	VM_5_DEC_CPU="$(grep 'Cpu:' "$VM_5_OUT" | awk '{print $2}')"
...
if [ `$VMRUN list | grep -w $VM_6 |wc -l` -eq "1" ]; then 
	VM_6_STATE="ON"
	VM_6_PERF="1"
	...
	VM_6_DEC_CPU="$(/usr/bin/top -n1 -b -c -p "$(pgrep -f "$VM_6_VMX")" | awk 'NR>7 {print $9}')"
...
#Added 6th VM  as  ${VM_6_PERF} if needed.
