...
# Enhanced by Michal Musil (majkal@centrum.cz)
...
print_usage() {
	...
	echo "-q: Name of the queue. If you don't select the queue, all will be applied"
	echo "-m: Max Job of the printer. Refer cupsd.conf. Set it if unset"
	echo ""
}
...
PRINTER_STATUS=`$LPSTAT -h $hostname -p $printername`

check_status()
{
	case $PRINTER_STATUS in
		*Rejecting*)
			MESSAGE="PRINTER: ${printername} is rejecting jobs. PRINT QUEUE: ${JOBCOUNT}/${maxjob} | $PERFDATA"
			exitstatus=$STATE_CRITICAL
		;;
        *Unable*)
			MESSAGE="PRINTER: Unabled to connect with CUPS. PRINT QUEUE: ${JOBCOUNT}/${maxjob} | $PERFDATA"
	        exitstatus=$STATE_CRITICAL
		;;
		*disabled*)
			MESSAGE="PRINTER: ${printername} is Stopped. PRINT QUEUE: ${JOBCOUNT}/${maxjob} | $PERFDATA" 
	        exitstatus=$STATE_CRITICAL
		;;
		*idle*)
			if [ "$JOBCOUNT" -ge "$critlevel" ]; then
				MESSAGE="PRINTER: ${printername}. PRINT QUEUE: ${JOBCOUNT}/${maxjob} | $PERFDATA"
        	 	exitstatus=$STATE_CRITICAL
        	elif [ "$JOBCOUNT" -ge "$warnlevel" ]; then
        		MESSAGE="PRINTER: ${printername}. PRINT QUEUE: ${JOBCOUNT}/${maxjob} | $PERFDATA"
                exitstatus=$STATE_WARNING
            else 
            	MESSAGE="PRINTER: ${printername} is idle. PRINT QUEUE: ${JOBCOUNT}/${maxjob} | $PERFDATA"
                exitstatus=$STATE_OK
            fi
		;;
        *printing*)
		...
		;;
		*)
			MESSAGE="Error occured while checking ${printername}. PRINT QUEUE: ${JOBCOUNT}/${maxjob} | $PERFDATA"
			exitstatus=$STATE_UNKNOWN
		;;
	esac
}
...
if [ -z $maxjob ]
then
	echo "You must specify a maxjob of printer using argument -m. Refer cupsd.conf"
	print_usage
	exitstatus=$STATE_UNKNOWN
	exit $exitstatus
fi
...
then
	...
	$LPSTAT -h $hostname -o $printername> $JOBTMP # run the lpstat command against the host.
	if [ $? -ne 0 ]
	then
	    rm -rf $JOBTMP
	...
	PERFDATA="print_jobs=$JOBCOUNT;$warnlevel;$critlevel;0;$maxjob"
