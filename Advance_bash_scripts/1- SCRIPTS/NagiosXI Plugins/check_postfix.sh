...
bounce=`( test -d hold && find bounce -type f ) | wc -l`
}

check_postfix_mailqueue
values="Sent ${active}, Sentlocal ${maildrop}, Deferred ${deferred}, Corrupt ${corrupt}, Hold ${hold}, Bounce ${bounce}, Incoming ${incoming}"
perfdata="sent=$active;; sentlocal=$maildrop;; deferred=$deferred;; corrupt=$corrupt;; hold=$hold;; bounce=$bounce;; incoming=$incoming;;"

if [ $corrupt -gt 0 ]; then
	echo -n "CRITICAL: Postfix Mailqueue: $corrupt corrupt messages found | $perfdata"
	exit $STATE_CRITICAL
fi

if [ $warning -gt 0 ] && [ $critical -gt 0 ]; then
   if [ $deferred -gt $critical ]; then
      echo -n "CRITICAL: Postfix Mailqueue: ${deferred} Deferred Mails | $perfdata"
      exit $STATE_CRITICAL
   elif [ $deferred -gt $warning ]; then
      echo -n "WARNING: Postfix Mailqueue: ${deferred} Deferred Mails | $perfdata"
      exit $STATE_WARNING
   else
      echo -n "OK: Postfix Mailqueue: $values | $perfdata"
      exit $STATE_OK
   fi
else
   echo -n "OK: Postfix Mailqueue: $values | $perfdata"
