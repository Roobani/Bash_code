...
#########################################################################
# Modified by john. removed the intervals value and graph.
#########################################################################
# ChangeLog:
#
# Version 1.4.0
...
if [ $UseRange = "True" ] ;then
	...
	if [ $Format"AA" = "SAA" ]; then
		...
		$Echo "$Msg" "-" Traffic IN \= "$uIn"${Unit_1}${Unit_2}, Traffic OUT \= "$uOut"${Unit_1}${Unit_2}, Total Traffic \= "$uTotal"${Unit_1}${Unit_2} \|In\=${uIn}${Unit_1}${Unit_2}\;\;\;0\;0 Out\=${uOut}${Unit_1}${Unit_2}\;\;\;0\;0 Total\=${uTotal}${Unit_1}${Unit_2}\;\;\;0\;0
	fi
	exit $Severity
	...
	if [ $Format"AA" = "SAA" ]; then
		...
		$Echo "$Msg" "-" Traffic IN \= "$uIn"${Unit_1}${Unit_2}, Traffic OUT \= "$uOut"${Unit_1}${Unit_2}, Total Traffic \= "$uTotal"${Unit_1}${Unit_2} \|In\=${uIn}${Unit_1}${Unit_2}\;${W1}\;${C1}\;0\;0 Out\=${uOut}${Unit_1}${Unit_2}\;${W2}\;${C2}\;0\;0 Total\=${uTotal}${Unit_1}${Unit_2}\;${Wt}\;${Ct}\;0\;0
	fi
	exit $Severity
fi
