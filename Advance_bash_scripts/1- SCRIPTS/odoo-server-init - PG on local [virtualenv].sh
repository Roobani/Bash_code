...
psql -d postgres --port=$DB_PORT -q -c "ALTER USER ${DBUSER} WITH SUPERUSER"
sleep 3
echo -en '\e[8;15r\e[8H'
...
psql -d postgres -q -c "ALTER USER ${DBUSER} WITH NOSUPERUSER"
psql -d postgres -q -c "ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO ${DBUSER}"
psql -d postgres -q -c "ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON SEQUENCES to ${DBUSER}"
psql -d postgres -q -c "ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON FUNCTIONS to ${DBUSER}"
psql -d postgres -q -c "ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TYPES to ${DBUSER}"
EOF3
PGOUT=$(basename $RDB)
date2=$(date +"%s")
diff=$(($date2-$date1))
