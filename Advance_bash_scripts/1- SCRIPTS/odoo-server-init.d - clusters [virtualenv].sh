...
PATH=/home/sodexis/virt_env/BV_10.0/bin:/bin:/sbin:/usr/bin
VIRTENV=/home/sodexis/virt_env/BV_10.0/bin/python
DAEMON=/opt/odoo/10.0/odoo.py
CHDIR=/opt/odoo/10.0/
NAME=odoo-server-10
DESC=odoo-server-10

# Odoo config path.
CONFIGPATH="/etc/odoo-server-10.conf"

#Odoo log file
LOG=/var/log/odoo/odoo-server.log

#Eclude table during DB dump. yes/no
EXCLUDE_TABLE=no

#Postgresql superuser connection to perform dbdump and db restore.
CONNECT_DB_IP='172.16.150.50'
CONNECT_DB_PORT=''
CONNECT_DB_NAME='postgres'
CONNECT_DB_USER='postgres'
CONNECT_DB_PASS='P1Es$soDe$'                     # The \  is the escape charater.

#Dont give / at end.
UPDATE_TMP_LOG_DIR=/opt/support_files/scripts/logs

#Wait time for -u all (in seconds). While doing -u all the Script wait to grep the UPDATE_CONTEXT within this time period to return as SUCCESS, else UNKNOWN, and terminate the -u all.
UPDATEINSEC="1200"
...
# ================================================================================================= #
# ======================== END OF VARIABLES, DON'T MODIFY ANYTHING BELOW ========================== #
# ================================================================================================= #
...
DATADIR=`cat "$CONFIGPATH" |grep data_dir |awk '{print $3}'`/filestore
...
# Setting pgpass, We need to run the entire script from root user. else we may see error: psql: fe_sendauth: no password supplied
#The pgpass need to be in /root/.pgpass .. sudo /etc/init.d/openerp-server start will write pgpass to /home/<sudo user>/.pgpass
# <databaseip>:<port>:<databasename>:<dbusername>:<password>          # The connect db name is * because during restore we connect to new created database so postgres user should conect any DB.
...
DBN=`psql -h $CONNECT_DB_IP -p $CONNECT_DB_PORT -d $CONNECT_DB_NAME -U $CONNECT_DB_USER -w --tuples-only -P format=unaligned -c "SELECT datname FROM pg_database JOIN pg_authid ON pg_database.datdba = pg_authid.oid WHERE rolname = '${ODOO_DB_USER}'"`
...
sessionclear() {
    ...
    echo -e " "
    psql -h ${CONNECT_DB_IP} -p ${CONNECT_DB_PORT} -d ${CONNECT_DB_NAME} -U ${CONNECT_DB_USER} -w -c "SELECT datid,datname,pid,usename,waiting,state,query FROM pg_stat_activity WHERE pg_stat_activity.datname = '$DB'"
    read -p "Do you need to close the sessions [y/n]?" char
    echo -e " "
    case "$char" in
        y|Y )
        psql -h ${CONNECT_DB_IP} -p ${CONNECT_DB_PORT} -d ${CONNECT_DB_NAME} -U ${CONNECT_DB_USER} -w -q -c "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '$DB' AND pid <> pg_backend_pid()"
        if [ "$?" -eq "0" ]; then
...
dropdb() {
    echo -e "Clearing the running sessions..."
    echo -e " "
    psql -h ${CONNECT_DB_IP} -p ${CONNECT_DB_PORT} -d ${CONNECT_DB_NAME} -U ${CONNECT_DB_USER} -w -q -c "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '$DB' AND pid <> pg_backend_pid()"
    if [ "$?" -eq "0" ]; then
        echo -e "Clearning sessions for '${DB}':\e[00;32m \033[5m SUCCESS \e[00m"
    ...
    psql -h ${CONNECT_DB_IP} -p ${CONNECT_DB_PORT} -d ${CONNECT_DB_NAME} -U ${CONNECT_DB_USER} -w -q -c "DROP DATABASE \"$DB\""
    sleep 5
    if [ -z `psql -h ${CONNECT_DB_IP} -p ${CONNECT_DB_PORT} -d ${CONNECT_DB_NAME} -U ${CONNECT_DB_USER} -w -tAc "SELECT 1 FROM pg_database WHERE datname = '$DB'"` ] && [ "$DB" != "all" ]; then
    ...
    echo -e " "
    echo -n "Cleaning the filestore for '${DB}'"
    cd ${DATADIR}/ && rm -rf ${DB}
...
renamed() {
    ...
    case "$char" in
        y|Y )
        psql -h ${CONNECT_DB_IP} -p ${CONNECT_DB_PORT} -d ${CONNECT_DB_NAME} -U ${CONNECT_DB_USER} -w -q -c "SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '$DB' AND pid <> pg_backend_pid()" >/dev/null 2>&1
        sleep 5
        psql -h ${CONNECT_DB_IP} -p ${CONNECT_DB_PORT} -d ${CONNECT_DB_NAME} -U ${CONNECT_DB_USER} -w -q -c
...
dumpdb () {
    ...
    if [ "$EXCLUDE_TABLE" = "yes" ]; then
        ...
        if ! pg_dump -U ${CONNECT_DB_USER} -h ${CONNECT_DB_IP} -p ${CONNECT_DB_PORT} -w -Fc -v -O -x ${EXCLUDE_TABLE_DATA} "$DB" > "$RDB"; then
        ...
        echo -en '\e[8;15r\e[8H'
        if ! pg_dump -U ${CONNECT_DB_USER} -h ${CONNECT_DB_IP} -p ${CONNECT_DB_PORT} -w -Fc -v -O -x "$DB" > "$RDB"; then
            echo -e "pg_dump: Error"
            echo -en '\e[r\e[6H'
            echo -e "Dump STATUS for Database '${DB}' with exclude-table=${EXCLUDE_TABLE}:"'\e[00;31m' "\033[5m FAILED \e[00m"
            echo -en '\e[r\e[16H'
...
restoredb () {
    ...
    if [[ ! -z `psql -h ${CONNECT_DB_IP} -p ${CONNECT_DB_PORT} -d ${CONNECT_DB_NAME} -U ${CONNECT_DB_USER} -w -tAc "SELECT 1 FROM pg_database WHERE datname = '$DB'"` ]]; then
        echo -e '\e[00;31m' "\033[5m ERROR \e[00m"
        echo " "
    ...
    date1=$(date +"%s")
psql -h ${CONNECT_DB_IP} -p ${CONNECT_DB_PORT} -d ${CONNECT_DB_NAME} -U ${CONNECT_DB_USER} -w -q <<EOF
CREATE DATABASE "${DB}" WITH OWNER ${DBUSER};
ALTER USER ${DBUSER} WITH SUPERUSER;
EOF
    sleep 3
    echo -en '\e[8;15r\e[8H'
    if ! PGPASSWORD=$PGPASS pg_restore -h $CONNECT_DB_IP -p $CONNECT_DB_PORT -U $DBUSER -e -w -v -j 4 -O -d "$DB" "$RDB"; then
        echo -e "pg_restore: Error"
        echo -en '\e[r\e[6H'
        echo -e "Restore STATUS for Database '${DB}':"'\e[00;31m' "\033[5m FAILED \e[00m"
        echo -en '\e[r\e[16H'
        psql -h ${CONNECT_DB_IP} -p ${CONNECT_DB_PORT} -d ${CONNECT_DB_NAME} -U ${CONNECT_DB_USER} -w -c "DROP DATABASE \"$DB\"" >/dev/null 2>&1
        exit 1
    else
    ...
    #The below is for any furture privilages setup. as the default privileges will be set for any new created tables/view/sequence.
psql -h ${CONNECT_DB_IP} -p ${CONNECT_DB_PORT} -d ${CONNECT_DB_NAME} -U ${CONNECT_DB_USER} -w -q <<EOF
...
EOF
    ...
    if [ $? -eq 0 ]; then
        ...
        DBSIZE="$(psql -h ${CONNECT_DB_IP} -p ${CONNECT_DB_PORT} -d ${CONNECT_DB_NAME} -U ${CONNECT_DB_USER} -w -tAc "SELECT pg_size_pretty(pg_database_size('$DB'))")"
