...
CONFIGPATH="/etc/openerp-server.conf"
...
OPENERP_DB_USER="$(grep 'db_user' $CONFIGPATH | awk '{ print $3 }')"
...
CHECK_LOG_LEVEL=$2
ERRORS=$3
LOG=/var/log/openerp/openerp-server.log
WORK="$(grep 'workers' $CONFIGPATH)"

CONFNAME=openerp-server.conf
...
UPDATE_CONTEXT="OpenERP server is running, waiting for connections..."

#Very important. for start-stop-daemon to start/stop openerp-server.
PIDFILE=/var/run/$NAME.pid
#pid=`cat $PIDFILE`

# END OF VARIABLES, NO NEED TO MODIFY ANYTHING BELOW ========================================================================================================================================== !!!

if [ ! -d /var/log/openerp ]; then
	mkdir -p /var/log/openerp
	chown ${USER}: /var/log/openerp
fi
...
declare -a DBNAME=(`psql -h ${CONNECT_DB_IP} -p ${CONNECT_DB_PORT} -d ${CONNECT_DB_NAME} -U ${CONNECT_DB_USER} -w --tuples-only -P format=unaligned -c "SELECT datname FROM pg_database JOIN pg_authid ON pg_database.datdba = pg_authid.oid WHERE rolname = '${OPENERP_DB_USER}'"`)
DBN=`psql -h ${CONNECT_DB_IP} -p ${CONNECT_DB_PORT} -d ${CONNECT_DB_NAME} -U ${CONNECT_DB_USER} -w --tuples-only -P format=unaligned -c "SELECT datname FROM pg_database JOIN pg_authid ON pg_database.datdba = pg_authid.oid WHERE rolname = '${OPENERP_DB_USER}'"`
...
dumpdb () {
    ...
    echo -e "\n"
    echo -en "Dump STATUS for Database '${DB}': "
    sleep 3
