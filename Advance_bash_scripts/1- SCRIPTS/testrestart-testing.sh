...
showmenu () {
    ...
    echo "1. Start the BPC Test compiere server"
    echo "2. Stop the BPC Test compiere server"
    echo "3. Restart the BPC Test compiere server"
    echo "4. Quit"
}
...
while true ; do
	...
	case "$choice" in
	    1)
		echo -e '\n'
		echo "Starting the BPC Test compiere server"
		sleep 3
		read -p "Continue (y/n)?" char
			...
			y|Y ) 
				...
				export LOG
				echo -e '\n'
				su compiere -c 'set -e && echo $EUID && cd /var/app/compiere/Compiere2/utils && sleep 5 && echo " " && echo -e "\e[00;32mExecuting RUN_Server2.sh\e[00m" && echo -e "\e[00;32m$divider\e[00m" && echo " " && sleep 5 && ./RUN_Server2.sh > server.log && sed '\''/Started in/q'\'' < <(tail -n 0 -f $LOG) && sleep 15 && echo " " && strt && sleep 5 && jconso && sleep 30'
				echo -e '\n'
				cd $DIRECTORY
				echo $EUID
				if fgrep -n -e '<link type="text/css" rel="stylesheet" href="https://testsstore.blueprintcleanse.com/gwtstore/downpanel.css">' -e '<script type="text/javascript" language="javascript" src="https://testsstore.blueprintcleanse.com/gwtstore/gwtstore.nocache.js">' index.html > out.txt; then
					...
					echo -e $divider '\n'
					sed -i -e 's_"https://testsstore.blueprintcleanse.com/gwtstore/downpanel.css"_"https://teststore.blueprintcleanse.com/gwtstore/downpanel.css"_g' index.html
					sed -i -e 's_"https://testsstore.blueprintcleanse.com/gwtstore/gwtstore.nocache.js"_"https://teststore.blueprintcleanse.com/gwtstore/gwtstore.nocache.js"_g' index.html
				else
					echo "$FILE is OK"
					echo "SCRIPT EXITS"
					exit 1
		...
		2)
		echo -e '\n'
		echo "Stopping the BPC Test compiere server"
		sleep 3
		read -p "Continue (y/n)?" char
			case "$char" in 
			y|Y ) 
		...
		3)
		echo -e '\n'	
		echo "Restarting the BPC Test compiere server"
		sleep 3
		read -p "Continue (y/n)?" char
			case "$char" in 
			y|Y )
